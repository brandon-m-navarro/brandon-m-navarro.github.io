...
export default class SlideControl extends BaseComponent {
  ...
  setDirection(direction) {
    ...
    if (direction != null && typeof direction !== "undefined") {
      ...
      if (!matched) {
        console.warn("WARN: Invalid SlideControl direction - " + direction);
      }
    }
  ...
  setTiming(timing) {
    ...
    if (timing != null && typeof timing !== "undefined") {
      ...
      if (
        timing.indexOf(timingFunctions) >= 0 ||
        timing.indexOf("cubic-bezier") >= 0
      ) {
        validTiming = timing;
      } else {
        console.warn("WARN: Invalid SlideControl timing function - " + timing);
      }
    }
  ...
  setContainerStyle() {
    ...
    if (this.options.duration != this.defaultOptions.duration) {
      this.div.style.transitionDuration = this.options.duration;
    }
    if (this.options.timing != this.defaultOptions.timing) {
      this.div.style.transitionTimingFunction = this.options.timing;
    }
